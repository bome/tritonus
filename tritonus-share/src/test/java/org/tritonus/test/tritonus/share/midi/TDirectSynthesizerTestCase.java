...
public class TDirectSynthesizerTestCase
{
			...
			int channel, Type type, int value1, int value2)
	{
		for (int i = 0; i < channels.length; i++)
		{
			...
			{
				assertEquals(
						type, ch.getType(), "affected channel: type");
				assertEquals(
						value1, ch.getValue1(), "affected channel: value1");
				assertEquals(
						value2, ch.getValue2(), "affected channel: value2");
			}
			else
			{
				assertEquals(
						Type.NONE, ch.getType(), "unaffected channel: type");
				assertEquals(
						-1, ch.getValue1(), "unaffected channel: value1");
				assertEquals(
						-1, ch.getValue2(), "unaffected channel: value2");
			}
		}
	}
