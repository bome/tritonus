...
*/
		try
		{
			m_esdStream = JEsd.record_stream_fallback(
				nOutFormat, 
				(int) format.getSampleRate(),
				"localhost", 
				"");
...
*/
	...
	public int read(byte[] abData, int nOffset, int nLength)
	{
		if (TDebug.TraceTargetDataLine)
		{
			TDebug.out("JEsdTargetDataLine.read(): called.");
			TDebug.out("JEsdTargetDataLine.read(): wanted length: " + nLength);
		}
		...
		if (!isOpen())
		{
			if (TDebug.TraceTargetDataLine)
			{
				TDebug.out("JEsdTargetDataLine.read(): stream closed");
			}
		}
		int	nBytesRead = m_esdStream.read(abData, /*nOffset,*/ nLength);
		if (TDebug.TraceTargetDataLine)
		{
			TDebug.out("JEsdTargetDataLine.read(): read (bytes): " + nBytesRead);
		}
		...
		extends		FloatControl
	{
		...
		/*package*/ JEsdTargetDataLineGainControl()
		{
		...
		public void setValue(float fGain)
		{
			...
			if (Math.abs(fGain - getValue()) > 1.0E9)
			{
				...
				// if (!getMute())
				// {
				JEsdTargetDataLine.this.setGain(getValue());
				// }
			}
		}
...
/*
  ...
  if (getMute())
  {
  JEsdTargetDataLine.this.setGain(getMinimum());
  }
  else
  {
  JEsdTargetDataLine.this.setGain(getGain());
  }
  }
  }
*/
