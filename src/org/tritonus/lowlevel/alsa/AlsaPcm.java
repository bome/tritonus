...
// 	public static final int	SND_PCM_DIG3_CON_CLOCK_VARIABLE =	(2<<4);	/* variable pitch */
	...
	static
	{
		...
		{
			System.out.println("AlsaPcm.<clinit>(): loading native library tritonusalsa");
		}
		System.loadLibrary("tritonusalsa");
		if (TDebug.TraceAlsaNative)
		{
			System.out.println("AlsaPcm.<clinit>(): loaded");
		}
	}
	 ...
	 */
	public native int getHWParamsPeriodSize(HWParams hwParams/* missing: ??? */);

	/**
	 *	Calls snd_pcm_hw_params_get_buffer_size().
	 */
	public native int getHWParamsBufferSize(HWParams hwParams);
...
//-------------------- from here: old methods ---------------------
	...
	*/
	...
	{
		/**
		 *	Holds the pointer to snd_pcm_hw_params_t for the native code.
		 *	This must be long to be 64bit-clean.
		 */
		/*private*/ long	m_lNativeHandle;
		...
		public native int getPeriodSize();
