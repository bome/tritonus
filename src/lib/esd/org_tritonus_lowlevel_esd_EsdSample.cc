/* 
 *	org_tritonus_lowlevel_esd_EsdSample.cc
 */
...
static int	getNativeFd(JNIEnv *env, jobject obj)
{
	...
	jfieldID	fid = env->GetFieldID(cls, "m_lNativeFd", "J");
...
{
	jclass	cls = env->GetObjectClass(obj);
	jfieldID	fid = env->GetFieldID(cls, "m_lNativeId", "J");
	return (int) env->GetLongField(obj, fid);
}
