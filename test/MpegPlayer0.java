	...
	extends		Thread
{
	...
	public MpegPlayer0(String sFilename)
	{
		super("player thread");
		...
		if (DEBUG)
		{
			System.out.println("MpegPlayer0.<init>(): source format: " + sourceFormat);
		}
		...
		if (DEBUG)
		{
			System.out.println("MpegPlayer0.<init>(): target format: " + targetFormat);
/*
...
*/
		...
		if (DEBUG)
		{
			System.out.println("MpegPlayer0.<init>(): received target format: " + m_format);
			System.out.println("MpegPlayer0.<init>(): end of contructor");
		}
	}
	...
	public void run()
	{
		if (DEBUG)
		{
			System.out.println("MpegPlayer0.run(): called");
		}
		...
		while (nBytesRead != -1)
		{
			...
			if (DEBUG)
			{
				System.out.println("MpegPlayer0.run(): read from AudioInputStream (bytes): " + nBytesRead);
			}
			if (nBytesRead >= 0)
			{
				...
				if (DEBUG)
				{
					System.out.println("MpegPlayer0.run(): written to SourceDataLine (bytes): " + nBytesWritten);
				}
			}
		}
		if (DEBUG)
		{
			System.out.println("MpegPlayer0.run(): after write loop");
		}
		//line.close();
	}
	...
	public static void main(String[] args)
	{
		if (args.length != 1)
		{
			System.out.println("MpegPlayer0: usage:");
			System.out.println("\tjava MpegPlayer0 <soundfile>");
		}
		else
		{
			Thread	thread = new MpegPlayer0(args[0]);
			thread.start();
		}
	}
}
